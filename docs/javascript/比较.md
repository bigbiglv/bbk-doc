# 值的比较
1. 不同类型在非严格相等 `=== !==` 比较的时候会做 [类型转换](./类型转换.md)，
2. 对象转原始值的时候 `ToPrimitive` 的 `hint` 为 `number`
3. `null` 在相等和大小比较有两个不同的规则，相等固定等于自身和 `undefined`， 比较会转数字得到 `0`
4. `undefined` 和其他值做相等和大小比较的时候都是 `false`, 相等比较只等于自身和 `null`
...
3. 非数字执行 [`SameValueNonNumber`](https://tc39.es/ecma262/multipage/abstract-operations.html#sec-samevaluenonnumber)
  1. 两侧为 `null` or `undefined` 恒等, 返回 `true`
    ```js
      null === null // true
...
2. 一侧是 `undefined` 一侧是 `null` 比较则返回 `true` （特殊处理）
